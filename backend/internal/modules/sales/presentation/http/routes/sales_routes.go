...
func RegisterSalesRoutes(router gin.IRouter, soHandler *handlers.SalesOrderHandler, siHandler *handlers.SalesInvoiceHandler, ptHandler *handlers.PosTransactionHandler, ooHandler *handlers.OnlineOrderHandler, csHandler *handlers.ConsignmentSalesHandler, srHandler *handlers.SalesReturnHandler, promoHandler *handlers.PromotionHandler, stHandler *handlers.SalesTargetHandler, skHandler *handlers.SalesKompetitorHandler, pmHandler *handlers.ProsesMarginHandler, srekHandler *handlers.SalesRekonsiliasiHandler) {
	sales := router.Group("/sales")
	{
		...
		{
			sk.POST("/", skHandler.CreateSalesKompetitor)
			sk.GET("/", skHandler.GetAllSalesKompetitors)
			sk.GET("/:id", skHandler.GetSalesKompetitorByID)
			sk.PUT("/:id", skHandler.UpdateSalesKompetitor)
			sk.DELETE("/:id", skHandler.DeleteSalesKompetitor)
		}

		// Proses Margin routes
		pm := sales.Group("/proses-margins")
		{
			pm.POST("/", pmHandler.CreateProsesMargin)
			pm.GET("/", pmHandler.GetAllProsesMargins)
			pm.GET("/:id", pmHandler.GetProsesMarginByID)
			pm.PUT("/:id", pmHandler.UpdateProsesMargin)
			pm.DELETE("/:id", pmHandler.DeleteProsesMargin)
		}

		// Sales Rekonsiliasi routes
		srek := sales.Group("/rekonsiliasi")
		{
			srek.POST("/", srekHandler.CreateSalesRekonsiliasi)
			srek.GET("/", srekHandler.GetAllSalesRekonsiliasi)
			srek.GET("/:id", srekHandler.GetSalesRekonsiliasiByID)
			srek.PUT("/:id", srekHandler.UpdateSalesRekonsiliasi)
			srek.DELETE("/:id", srekHandler.DeleteSalesRekonsiliasi)
		}
	}
}
