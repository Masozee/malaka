...
// GetAllOnlineOrders handles retrieving all online orders.
func (h *OnlineOrderHandler) GetAllOnlineOrders(c *gin.Context) {
	orders, err := h.service.GetAllOnlineOrders(c.Request.Context())
	if err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}

	response.OK(c, "Online orders retrieved successfully", orders)
}
