...
// GetAllSalesReturns handles retrieving all sales returns.
func (h *SalesReturnHandler) GetAllSalesReturns(c *gin.Context) {
	srs, err := h.service.GetAllSalesReturns(c.Request.Context())
	if err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}

	response.OK(c, "Sales returns retrieved successfully", srs)
}
