...
// GetAll retrieves all consignment sales from the database.
func (r *ConsignmentSalesRepositoryImpl) GetAll(ctx context.Context) ([]*entities.ConsignmentSales, error) {
	query := `SELECT id, consignee_id, sales_date, total_amount, status, created_at, updated_at FROM consignment_sales`
	...
	var css []*entities.ConsignmentSales
	for rows.Next() {
		cs := &entities.ConsignmentSales{}
		if err := rows.Scan(&cs.ID, &cs.ConsigneeID, &cs.SalesDate, &cs.TotalAmount, &cs.Status, &cs.CreatedAt, &cs.UpdatedAt); err != nil {
			return nil, err
		}
		css = append(css, cs)
	}
