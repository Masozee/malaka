...
func (r *cashBookRepositoryImpl) GetByID(ctx context.Context, id string) (*entities.CashBook, error) {
...
func (r *cashBookRepositoryImpl) Delete(ctx context.Context, id string) error {
	query := `DELETE FROM cash_books WHERE id = $1`
	_, err := r.db.ExecContext(ctx, query, id)
	return err
}

func (r *cashBookRepositoryImpl) GetByCashBankID(ctx context.Context, cashBankID string) ([]*entities.CashBook, error) {
	query := `
		SELECT id, cash_bank_id, transaction_date, reference_number, description,
			   debit_amount, credit_amount, balance, transaction_type, source_module,
			   source_id, created_by, created_at, updated_at
...
func (r *cashBookRepositoryImpl) GetByDateRange(ctx context.Context, cashBankID string, startDate, endDate time.Time) ([]*entities.CashBook, error) {
	query := `
		SELECT id, cash_bank_id, transaction_date, reference_number, description,
			   debit_amount, credit_amount, balance, transaction_type, source_module,
			   source_id, created_by, created_at, updated_at
