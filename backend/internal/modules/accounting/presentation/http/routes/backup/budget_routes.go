...
// BudgetRoutes sets up the routes for budgets
func BudgetRoutes(router *gin.RouterGroup, handler *handlers.BudgetHandler) {
	budgets := router.Group("/budgets")
	{
		...
		budgets.GET("/period/:company_id/:start_date/:end_date", handler.GetBudgetsByPeriod)
		...
		budgets.GET("/:id/comparison/:as_of_date", handler.GetBudgetComparison)
		budgets.POST("/:id/update-actuals", handler.UpdateActualAmounts)
		budgets.GET("/variance-report/:company_id/:budget_type/:as_of_date", handler.GetBudgetVarianceReport)
		budgets.GET("/:id/utilization", handler.GetBudgetUtilization)
		budgets.GET("/:id/performance/:company_id/:fiscal_year", handler.GetBudgetPerformance)
		
		// Batch operations
		budgets.POST("/with-lines", handler.CreateBudgetWithLines)
		budgets.PUT("/with-lines", handler.UpdateBudgetWithLines)
		...
		// Budget forecasting
		budgets.POST("/:id/forecast/:projection_months", handler.ForecastBudget)
		budgets.GET("/year-over-year/:company_id/:budget_type/:current_year/:previous_year", handler.CompareBudgetYearOverYear)
	}
}
