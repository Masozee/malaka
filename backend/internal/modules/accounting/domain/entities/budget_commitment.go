...
type BudgetCommitment struct {
	...
	CommittedBy     uuid.ID                       `json:"committed_by" db:"committed_by"`
	CommittedAt     time.Time                     `json:"committed_at" db:"committed_at"`
	ReleasedAt      *time.Time                    `json:"released_at,omitempty" db:"released_at"`
	ReleasedBy      *uuid.ID                      `json:"released_by,omitempty" db:"released_by"`
...
func NewBudgetCommitment(budgetID, accountID, referenceID, committedBy uuid.ID, amount float64, refType BudgetCommitmentReferenceType, refNumber, description string) *BudgetCommitment {
	now := time.Now()
...
func (bc *BudgetCommitment) Release(releasedBy uuid.ID, reason string) {
	now := time.Now()
