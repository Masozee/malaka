...
type BudgetRealization struct {
	...
	CommitmentID    *uuid.UUID                     `json:"commitment_id,omitempty" db:"commitment_id"`
	...
	RealizedBy      uuid.UUID                      `json:"realized_by" db:"realized_by"`
...
func NewBudgetRealization(budgetID, accountID, referenceID, realizedBy uuid.UUID, amount float64, refType BudgetRealizationReferenceType, refNumber, description string, transactionDate time.Time, commitmentID *uuid.UUID) *BudgetRealization {
	now := time.Now()
