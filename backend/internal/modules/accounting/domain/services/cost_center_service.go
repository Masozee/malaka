...
type CostCenterService interface {
	...
	GetCostCenterByID(ctx context.Context, id uuid.ID) (*entities.CostCenter, error)
	...
	DeleteCostCenter(ctx context.Context, id uuid.ID) error

	// Cost center allocation operations
	CreateAllocation(ctx context.Context, allocation *entities.CostCenterAllocation) error
	GetAllocationByID(ctx context.Context, allocationID uuid.ID) (*entities.CostCenterAllocation, error)
	...
	GetCostCentersByParent(ctx context.Context, parentID uuid.ID) ([]*entities.CostCenter, error)
	...
	DeactivateCostCenter(ctx context.Context, costCenterID uuid.ID) error
	ReactivateCostCenter(ctx context.Context, costCenterID uuid.ID) error
	...
	ProcessAllAllocations(ctx context.Context, costCenterID uuid.ID, period time.Time) error
	...
	GetTotalCosts(ctx context.Context, costCenterID uuid.ID, startDate, endDate time.Time) (map[string]float64, error)
	
	// Hierarchical operations
	GetCostCenterChildren(ctx context.Context, parentID uuid.ID) ([]*entities.CostCenter, error)
	GetCostCenterDescendants(ctx context.Context, parentID uuid.ID) ([]*entities.CostCenter, error)
	GetCostCenterPath(ctx context.Context, costCenterID uuid.ID) ([]*entities.CostCenter, error)
	GetCostCenterLevel(ctx context.Context, costCenterID uuid.ID) (int, error)
	...
	CheckCircularReference(ctx context.Context, costCenterID, parentID uuid.ID) error
	
	// Performance analytics
	GetCostCenterEfficiency(ctx context.Context, costCenterID uuid.ID, period time.Time) (map[string]float64, error)
	CompareCostCenters(ctx context.Context, costCenterIDs []uuid.ID, startDate, endDate time.Time) (map[uuid.ID]*entities.CostCenterReport, error)
	...
	ValidateAllocationPercentages(ctx context.Context, sourceCostCenterID uuid.ID, period time.Time) error
	ProcessMonthlyAllocations(ctx context.Context, companyID string, period time.Time) error
	RecalculateAllocations(ctx context.Context, costCenterID uuid.ID, period time.Time) error
	
	// Cost center setup and management
	CreateCostCenterHierarchy(ctx context.Context, hierarchy []*entities.CostCenter) error
	MoveCostCenter(ctx context.Context, costCenterID, newParentID uuid.ID) error
	MergeCostCenters(ctx context.Context, sourceCostCenterID, targetCostCenterID uuid.ID) error
	SplitCostCenter(ctx context.Context, costCenterID uuid.ID, newCostCenters []*entities.CostCenter) error
}