...
func (h *GalleryImageHandler) GetGalleryImageByID(c *gin.Context) {
	id := c.Param("id")
	image, err := h.service.GetGalleryImageByID(c.Request.Context(), uuid.MustParse(id))
	if err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}
...
func (h *GalleryImageHandler) UpdateGalleryImage(c *gin.Context) {
	...
	// First, retrieve the existing gallery image
	existingImage, err := h.service.GetGalleryImageByID(c.Request.Context(), uuid.MustParse(id))
	if err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}
	...
	req.ApplyToEntity(existingImage)

	if err := h.service.UpdateGalleryImage(c.Request.Context(), existingImage); err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}

	response.OK(c, "Gallery image updated successfully", existingImage)
}

// DeleteGalleryImage handles deleting a gallery image by its ID.
func (h *GalleryImageHandler) DeleteGalleryImage(c *gin.Context) {
	id := c.Param("id")
	if err := h.service.DeleteGalleryImage(c.Request.Context(), uuid.MustParse(id)); err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}
...
func (h *GalleryImageHandler) GetGalleryImagesByArticleID(c *gin.Context) {
	...
	images, err := h.service.GetGalleryImagesByArticleID(c.Request.Context(), uuid.MustParse(articleID))
	if err != nil {
		response.InternalServerError(c, err.Error(), nil)
		return
	}
