...
export default function InputTaxPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const data = await taxTransactionService.getAll('input')
                setTransactions(data)
            } catch (error) {
                console.error('Failed to fetch input tax transactions:', error)
            } finally {
                setLoading(false)
            }
        }
    ...
    const stats = useMemo(() => {
        ...
        return { totalVAT, totalBase, transactionCount: transactions.length }
    }, [transactions])
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex flex-col">
                    <span className="font-medium text-sm">{row.original.reference_type}</span>
                    <span className="text-xs text-muted-foreground">{row.original.reference_number}</span>
                </div>
            )
        },
        {
            ...
            <div className="flex-1 p-6 space-y-6">
                    ...
                    <Card>
                        <CardContent className="p-4 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-green-600">Rp {(stats.totalBase / 1000000).toFixed(1)}M</p>
                            </div>
                ...
                {loading ? (
                    <div className="flex items-center justify-center py-12">
                        <p className="text-muted-foreground">Loading input tax data...</p>
                    </div>
                ) : (
                    <TanStackDataTable
                        data={filteredData}
