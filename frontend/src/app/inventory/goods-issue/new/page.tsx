...
import { goodsIssueService } from '@/services/inventory'
...
export default function NewGoodsIssuePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      await goodsIssueService.createIssue({
      ...
      addToast({ type: 'success', title: 'Goods issue created successfully' })
      router.push('/inventory/goods-issue')
    } catch (err) {
      console.error('Error creating goods issue:', err)
      addToast({ type: 'error', title: 'Failed to create goods issue' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <TwoLevelLayout>
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        {/* Page title */}
        <div className="-mt-2">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">New Goods Issue</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Create a new outgoing inventory issue</p>
        </div>
